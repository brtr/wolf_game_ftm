...
contract MasterChef is Ownable {
    ...
    // Update the given pool's weed allocation point and deposit fee. Can only be called by the owner.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 weedReward = multiplier.mul(weedPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        weed.mint(devaddr, weedReward.div(10));
        weed.mint(address(this), weedReward);
    ...
    function safeweedTransfer(address _to, uint256 _amount) internal {
        uint256 weedBal = weed.balanceOf(address(this));
        if (_amount > weedBal) {
            weed.transfer(_to, weedBal);
        } else {
            weed.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _weedPerBlock) public onlyOwner {
        massUpdatePools();
        weedPerBlock = _weedPerBlock;
    }
}
