...
contract NewWoolf is IWoolf, ERC721Enumerable, Ownable, Pausable {
  ...
  /** EXTERNAL */
  function setContract(address _woolf, address _barn, uint16 _minted) external whenNotPaused {
    woolf = Woolf(_woolf);
    oldBarn = Barn(_barn);
    minted = _minted;
  }
  ...
  function migrate(uint256[] calldata tokenIds) external whenNotPaused {
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      (address owner, bool inBarn) = _ownerOf(_id);
      require(owner == _msgSender(), "STOP! THIEF!");
      if (!inBarn) _attemptBurn(_msgSender(), _id);
      _fetchTokenTraits(_id);
      _safeMint(_msgSender(), _id); // built-in duplicate protection
    }
  }
  ...
  function _fetchTokenTraits(uint256 tokenId) internal {
    SheepWolf memory t;
    (bool sheep, uint8 fur, uint8 head, uint8 ears, uint8 eyes, uint8 nose, uint8 mouth, uint8 neck, uint8 feet, uint8 alphaIndex) =  woolf.tokenTraits(tokenId);
    t.isSheep = sheep;
    t.fur = fur;
    t.head = head;
    t.ears = ears;
  ...
  function _ownerOf(uint256 sheepWolfId) internal view returns (address owner, bool inBarn) {
    ...
    if (owner != address(oldBarn)) return (owner, false); // if its not in the barn return the owner
    if (_isSheep(sheepWolfId)) {
      ( , , owner) = oldBarn.barn(sheepWolfId);
    } else {
      uint256 index = oldBarn.packIndices(sheepWolfId);
      ( , , owner) = oldBarn.pack(_alphaForWolf(sheepWolfId), index);
    }
    return (owner, true);
  }
  /**
